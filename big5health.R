...
ipip.l <- ipip.l %>% 
  separate(traits, into=c('trait', 'item'), sep="_")

# Calculate averages for each participant (coded as RID) and trait:
ipip.comp <- ipip.l %>% 
  group_by(RID, trait) %>% 
  summarise(trait_avg=mean(score))
...
ipip.comp$exer <- ipip.comp$exer %>%
  factor(levels=c('veryRarelyNever', 'less1mo', 'less1wk', '1or2wk', '3or5wk', 'more5wk' ))
...
exer.avg <- ipip.comp %>% 
  group_by(exer, trait) %>%
  summarise(avg=mean(trait_avg), sem=(sd(trait_avg)/sqrt(1000)))
...
ipip.comp <- ipip.comp %>%
  mutate(BMI_cat= case_when(BMI<18.5 ~ 'underweight', 
            ...
            BMI>30 ~ 'obese', TRUE ~ as.character(BMI)))

# turn BMI_cat into a factor and order it with levels
ipip.comp$BMI_cat <- ipip.comp$BMI_cat %>%
  factor(levels=c('underweight', 'healthy', 'overweight'))
...
age.avg <- ipip.comp %>% 
  group_by(trait) %>%
  summarise(corrcoef=cor(age,trait_avg))
